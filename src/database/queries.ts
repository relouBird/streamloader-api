...
export const queries = {
  ...
  async createUser(id: string, email: string, password: string): Promise<void> {
    await db.execute(`INSERT INTO users (id, email, password) VALUES (?,?,?)`, [
      id,
      email,
      password,
    ]);
    ...
    const [rows] = await db.execute(
      `SELECT id, email, password,
              CASE
                ...
                ELSE plan
              END AS plan,
              premium_until, trim_trials_used, created_at, updated_at
       FROM users
       WHERE email = ?`,
      [email],
    );
    ...
    const [rows] = await db.execute(
      `SELECT id, email,
              CASE
                ...
                ELSE plan
              END AS plan,
              premium_until, trim_trials_used, created_at
       FROM users
       WHERE id = ?`,
      [id],
    );
      ...
      `UPDATE users
       SET plan='premium',
           premium_until = datetime(
             CASE
               WHEN premium_until IS NOT NULL AND premium_until > datetime('now')
               THEN premium_until
               ELSE datetime('now')
             END,
             '+' || ? || ' months'
  ...
  async incrementTrimTrial(id: string): Promise<void> {
    await db.execute(
      `UPDATE users
       SET trim_trials_used = trim_trials_used + 1,
           updated_at = datetime('now')
       WHERE id = ?`,
      [id],
    );
  ...
  async createTransaction(
    ...
    await db.execute(
      `INSERT INTO transactions (id, user_id, provider, amount, plan)
       VALUES (?,?,?,?,?)`,
  ...
  async logDownload(
    ...
    const [result] = await db.execute(
      `INSERT INTO downloads_log (user_id, client_key, url, format, title, subtitled)
       VALUES (?,?,?,?,?,?)`,
  ...
  async updateDownloadStatus(id: number, status: string): Promise<void> {
    await db.execute(`UPDATE downloads_log SET status=? WHERE id=?`, [
      status,
      id,
    ]);
  },
  ...
  async createReview(
    ...
    await db.execute(
      `INSERT INTO reviews (client_key, rating, comment) VALUES (?,?,?)`,
  ...
  async approveReview(id: number): Promise<void> {
    await db.execute(`UPDATE reviews SET status='approved' WHERE id=?`, [id]);
  },

  async rejectReview(id: number): Promise<void> {
    await db.execute(`UPDATE reviews SET status='rejected' WHERE id=?`, [id]);
  },
  ...
  async getReviewStats(): Promise<ReviewStats> {
    ...
    return first<ReviewStats>(rows) ?? { count: 0, average: null };
  },
